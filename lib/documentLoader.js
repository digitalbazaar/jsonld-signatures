...
api.extendContextLoader = documentLoader => {
   ...
   * cache, it returns the context otherwise it uses the curried documentLoader
   * to fetch a jsonld context from a remote source.
   *
   ...
   *
   * @returns {Function} A function that accepts a url to a jsonld context.
   */
  return async url => {
    ...
    try {
      const remoteContext = await documentLoader(url);
      return remoteContext;
    } catch(e) {
      console.error(url, e);
      console.warn(`Unable to load context ${url}`);
      throw e;
    }
  };
};
