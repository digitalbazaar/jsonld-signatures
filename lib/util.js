  ...
  if(env.nodejs) {
    return (b1, b2) => {
      const buffer = Buffer.concat([
        Buffer.from(b1.buffer, 0, b1.length),
        Buffer.from(b2.buffer, 0, b2.length)]);
      return new Uint8Array(buffer.buffer, 0, buffer.length);
    };
  }
  return (b1, b2) => {
    const rval = Uint8Array(b1.length + b2.length);
    rval.set(b1, 0);
  ...
  if(env.nodejs) {
    return ({encodedHeader, verifyData}) => {
      const buffer = Buffer.concat([
        ...
        Buffer.from(verifyData.buffer, 0, verifyData.length)
      ]);
      const data = new Uint8Array(buffer.buffer, 0, buffer.length);
      return {data};
    };
  }
  return ({encodedHeader, verifyData}) => {
    ...
    const data = forge.util.binary.raw.decode(buffer.getBytes());
    return {data};
  };
}
...
function base64urlEncodeFactory() {
  if(env.nodejs) {
    ...
    return data => base64url(Buffer.from(data.buffer, 0, data.length));
  }
  return data => {
...
function base64urlDecodeFactory() {
  if(env.nodejs) {
    const base64url = require('base64url');
    return string => {
      const buffer = base64url.decode(string);
      return new Uint8Array(buffer.buffer, 0, buffer.length);
    };
  }
  return string => {
