...
api.sign = async function sign(document, {
  ...
  // flag to false).
  _ensureSuiteContext({document, suite, addSuiteContext});
 ...
 * @param {LinkedDataSignature} options.suite - Signature suite instance.
 * @param {boolean} options.addSuiteContext - Add suite context?
 */
function _ensureSuiteContext({document, suite, addSuiteContext}) {
  const {contextUrl} = suite;

  // Handle the unfortunate Ed25519Signature2018 / credentials/v1 collision
  const ED_2018_CONTEXT = 'https://w3id.org/security/suites/ed25519-2018/v1';
  const CRED_CONTEXT = 'https://www.w3.org/2018/credentials/v1';
  if(suite.type === 'Ed25519Signature2018') {
    const hasEd2018 = _includesContext({document, contextUrl: ED_2018_CONTEXT});
    const hasCred = _includesContext({document, contextUrl: CRED_CONTEXT});

    if(hasEd2018 && hasCred) {
      // Warn if both are present
      console.warn('Warning: The ed25519-2018/v1 and credentials/v1 contexts' +
        ' are incompatible.');
      console.warn('For VCs, using the credentials/v1 context is sufficient.');
      return;
    }
    if(hasEd2018 || hasCred) {
