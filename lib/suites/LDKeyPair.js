...
function ed25519SignerFactory(key) {
  if(env.nodejs) {
    ...
    return {
      async sign({data}) {
        return chloride.crypto_sign_detached(data, privateKey);
      }
    };
  }
    ...
    return {
      async verify({data, signature}) {
        return chloride.crypto_sign_verify_detached(
          Buffer.from(signature.buffer, 0, signature.length),
          Buffer.from(data.buffer, 0, data.length),
          publicKey);
      }
    };
  }
    ...
    if('RSA_PKCS1_PSS_PADDING' in crypto.constants) {
      return {
        async sign({data}) {
          ...
          signer.update(Buffer.from(data.buffer, 0, data.length));
    ...
    if('RSA_PKCS1_PSS_PADDING' in crypto.constants) {
      return {
        async verify({data, signature}) {
          ...
          verifier.update(Buffer.from(data.buffer, 0, data.length));
