...
module.exports = class LinkedDataSignature {
  ...
  async sign(input, options) {
    ...
    // and handler *must* maintain this form. Handlers that introduce terms that
    // are not in the SECURITY_CONTEXT *must* compact the proof using the
    // SECURITY_CONTEXT before returning.
    const {purpose} = options;
    if(purpose) {
      const {proofPurposeHandler} = purpose;
      proof = await proofPurposeHandler.createProof({proof, purpose});
    }
  ...
  async verify(framed, options) {
    ...
    const {purpose} = options;
    if(purpose) {
      const {expanded, purpose: proofPurposeHandler} = options;
      const result = await proofPurposeHandler.verify(
        {document: expanded, proof, purpose});
      if(!result.verified) {
        throw result.error;
      }
    }
    ...
    return this.verifyProofNode(
      verifyData, proof,
      Object.assign({}, options, {publicKey}));
  }
