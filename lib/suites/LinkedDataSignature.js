...
const UNDEFINED_SIGNER = {
  sign: () => {
    throw new Error('A signer was not specified for the signature suite.');
  }
};

const UNDEFINED_VERIFIER = {
  verify: () => {
    throw new Error('A verifier was not specified for the signature suite.');
  }
};
...
function _processSignatureParams({key, signer, verifier}) {
  ...
  if(key) {
    ...
    } else if(key.signer === undefined) {
      vm.signer = UNDEFINED_SIGNER;
    }

    if(typeof key.verifier === 'function') {
      vm.verifier = key.verifier();
    } else if(key.verifier === undefined) {
      vm.verifier = UNDEFINED_VERIFIER;
    }
    ...
    vm.signer = signer || UNDEFINED_SIGNER;
    vm.verifier = verifier || UNDEFINED_VERIFIER;
    vm.verificationMethod = (signer && signer.id) || (verifier && verifier.id);

    if(!(signer || verifier)) {
      throw new TypeError(
        'Neither a "signer" or a "verifier" was specified.');
    }
  }
