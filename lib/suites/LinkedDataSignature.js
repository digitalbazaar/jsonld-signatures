...
module.exports = class LinkedDataSignature extends LinkedDataProof {
   ...
   *
   * @param {boolean} [addSuiteContext=true] - Toggles the default behavior of
   *   each signature suite enforcing the presence of its own `@context` (if
   *   it is not present, it's added to the context list).
   *
  ...
  constructor({
    ...
    useNativeCanonize, addSuiteContext = true
  } = {}) {
    super({type});
    ...
    this.addSuiteContext = addSuiteContext;
    if(date) {
