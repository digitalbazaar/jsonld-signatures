...
module.exports = class LinkedDataSignature extends LinkedDataProof {
   ...
   *
   * @param {string} [options.verificationMethod]  A key id URL of the paired
   *   public key. If not provided, this is set from key.id, signer.id or
   *   verifier.id.
   ...
   */
  constructor({
    type, verificationMethod, proof, LDKeyClass, date, key, signer, verifier,
    useNativeCanonize
  } = {}) {
    super({type});
    this.LDKeyClass = LDKeyClass;
    this.proof = proof;
    const vm = this._validateVerificationMethod({
      verificationMethod, key, signer, verifier
    });
   ...
   */
  _validateVerificationMethod({verificationMethod, key, signer, verifier}) {
    if(!(key || signer || verifier)) {
      // eslint-disable-next-line max-len
      throw new TypeError('Either a "key" or at least one of "signer"/"verifier" parameters is required');
    }
    if((key && signer) || (key && verifier)) {
      // eslint-disable-next-line max-len
      throw new TypeError('Either a "key" or at least one of "signer"/"verifier" parameters is required, but not both.');
    }

    const vm = {};
    if(key) {
      vm.key = key;
      vm.verificationMethod = verificationMethod || key.id;
      ...
      vm.verificationMethod = verificationMethod ||
        signer && signer.id ||
    ...
    if(!(vm.verificationMethod && typeof verificationMethod !== 'string')) {
