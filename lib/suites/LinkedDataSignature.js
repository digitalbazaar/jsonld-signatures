...
module.exports = class LinkedDataSignature {
  ...
  async sign(input, options) {
    ...
    if(proofPurposeHandler) {
      proof = await proofPurposeHandler.createProof({proof, purposeParameters});
    }
  ...
  async verify(framed, options) {
    ...
    if(purpose) {
      ...
      const result = await proofPurposeHandler.validate(
        {document: framed, proof, purposeParameters, documentLoader});
      if(!result.verified) {
        throw result.error;
      }
    }
