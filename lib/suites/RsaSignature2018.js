...
module.exports = class RsaSignature2018 extends LinkedDataSignature {
   ...
   * @param [options.privateKeyPem] {string} PEM encoded private key
   */
  constructor(injector, options = {}) {
    super(injector, {algorithm: 'RsaSignature2018', ...options});
   ...
   *
   * @param [options.privateKeyPem] {string} Optional PEM encoded private key,
   *   if it was not already passed in suite constructor.
   *
   * @returns {Promise<string>} Signature as an encoded detached JWS
   */
  async createSignatureValue(inputData, options = {}) {
    ...
    */
    ...
    if(this.injector.env.nodejs) {
      ...
      if('RSA_PKCS1_PSS_PADDING' in crypto.constants) {
        ...
        const {data} = util.createJws(
          {encodedHeader, forge, nodejs, verifyData: inputData});
    ...
    if(!encodedSignature) {
      ...
      md.update(inputData.data, inputData.encoding);
  ...
  async verifyProofNode(verifyData, proof, options) {
    ...
    if(this.injector.env.nodejs) {
      ...
      if('RSA_PKCS1_PSS_PADDING' in crypto.constants) {
        ...
        verifier.update(Buffer.from(verifyData.data, verifyData.encoding));
        return verifier.verify({
          key: publicKeyPem,
          ...this.createPss()
        }, Buffer.from(rawSignature, 'binary'));
      }
    }
